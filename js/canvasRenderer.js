...
function renderSticker(sticker) {
  ...
  if (sticker.color) {
    // 색상이 지정된 경우 (offscreen canvas를 사용하여 tinting)
    const offscreenCanvas = document.createElement('canvas');
    offscreenCanvas.width = sticker.width;
    offscreenCanvas.height = sticker.height;
    const offCtx = offscreenCanvas.getContext('2d');

    // 1. 이미지 그리기
    offCtx.drawImage(sticker.image, 0, 0, sticker.width, sticker.height);

    // 2. 소스 이미지가 있는 곳에만 색상 덮어쓰기 (source-in)
    offCtx.globalCompositeOperation = 'source-in';
    offCtx.fillStyle = sticker.color;
    offCtx.fillRect(0, 0, sticker.width, sticker.height);

    // 3. 결과물을 메인 캔버스에 그리기 (중앙 정렬)
    ctx.drawImage(
      offscreenCanvas,
      -sticker.width / 2,
      -sticker.height / 2,
...
export function findStickerResizeHandle(x, y) {
  ...
  const padding = 8;
  // 손가락으로도 잡을 수 있도록 히트 영역을 시각 핸들보다 넉넉하게
  const hit = HANDLE_HIT_RADIUS;
