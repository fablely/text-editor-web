...
function openFontPicker(selectorObj) {
  ...
  // 현재 폰트를 미리 로드해 피커에서 바로 보이도록
  ensureFontLoaded(fontOptions[selectedFontIndex].value);
...
function previewFontSelection() {
  ...
  const value = fontOptions[selectedFontIndex].value;
  const text = fontOptions[selectedFontIndex].text;
  ...
  if (display) {
    ...
    display.style.fontFamily = value;
  }

  // 해당 폰트를 이 시점에 1회 로드 → 로드 완료 후 미리보기 반영
  ensureFontLoaded(value).then(() => {
    // 선택된 텍스트가 있으면 폰트 변경 적용
    if (state.selectedText) {
      state.selectedText.fontFamily = value;

      // 모달 폰트 선택기 동기화
      document.getElementById('modalFontFamily').value = value;
      const modalFontDisplay = document.getElementById('modalFontDisplay');
      if (modalFontDisplay) {
        modalFontDisplay.textContent = text;
        modalFontDisplay.style.fontFamily = value;
      }
...
function applyFontSelection() {
  ...
  if (state.selectedText) {
    ...
    // 모달 폰트 선택기 동기화
    document.getElementById('modalFontFamily').value = value;
    document.getElementById('modalFontDisplay').textContent = text;
  ...
  log(`최종 선택 폰트: ${fontOptions[selectedFontIndex].text}`);
}
